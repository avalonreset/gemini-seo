# Firecrawl Extension Installer for Gemini SEO (Windows)
...
if (-not (Test-Path $SeoSkillDir)) {
    Write-Host "x Gemini SEO is not installed." -ForegroundColor Red
    Write-Host "  Install it first: irm https://raw.githubusercontent.com/avalonreset/gemini-seo/main/install.ps1 | iex"
    exit 1
}
Write-Host "v Gemini SEO detected" -ForegroundColor Green
