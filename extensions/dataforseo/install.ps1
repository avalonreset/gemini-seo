# DataForSEO Extension Installer for Gemini SEO (Windows)
...
Write-Host "║   For Gemini SEO                     ║" -ForegroundColor Cyan
Write-Host "════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $SeoSkillDir)) {
    Write-Host "✗ Gemini SEO is not installed." -ForegroundColor Red
    Write-Host "  Install it first: irm https://raw.githubusercontent.com/avalonreset/gemini-seo/main/install.ps1 | iex"
    exit 1
}
Write-Host "✓ Gemini SEO detected" -ForegroundColor Green
...
if (Test-Path "$ScriptDir\skills\seo-dataforseo\SKILL.md") {
    ...
    Write-Host "  Run this script from the gemini-seo repo."
    exit 1
}
...
Write-Host "  1. Restart your agent CLI"
